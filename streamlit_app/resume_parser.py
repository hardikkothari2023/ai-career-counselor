...
# # ❌ Disable Tesseract completely for Streamlit Cloud
# pytesseract = None
...
#     """
#     FAST & CLOUD SAFE:
#     Extract selectable text directly from the PDF using PyMuPDF.
#     No OCR, no EasyOCR, no crashing.
#     """
...
#     with fitz.open(stream=file_obj.read(), filetype="pdf") as doc:
#         for page in doc:
#             page_text = page.get_text()  # DIRECT TEXT EXTRACTION
#             if page_text.strip():
...
# ---- Disable all OCR for Streamlit Cloud ----
# Streamlit Cloud cannot download EasyOCR models or run Tesseract
USE_OCR = False
...
def extract_text_from_pdf(file_obj):
    ...
    ✔ No model loading
    ✔ No Streamlit crash
    Extracts selectable text directly from the PDF.
    """
    text_list = []
    file_obj.seek(0)
    ...
    with fitz.open(stream=file_obj.read(), filetype="pdf") as doc:
        for page in doc:
            page_text = page.get_text()  # DIRECT TEXT (no OCR)
...
def clean_text_before_name_extraction(text):
    ...
    for line in text.split("\n"):
        ...
        # Skip headings
        if low in {"summary", "education", "skills", "projects", "experience", "contact"}:
            continue

        # Likely name lines should be short
...
def extract_name(text):
    """
    Extract the most likely PERSON name using spaCy + heuristics.
    """
    if not text.strip():
        return "Unknown"
    ...
    for ent in doc.ents:
        if ent.label_ == "PERSON":
            nm = ent.text.strip()
            if all(w.lower() not in INVALID for w in nm.split()):
                candidates.add(nm)

    lines = [l.strip() for l in text.split("\n") if l.strip()]

    # Top 5 lines heuristic
    for ln in lines[:5]:
        if 1 <= len(ln.split()) <= 4 and not re.search(r"\d", ln):
            candidates.add(ln)

    # Regex fallback
    regex_matches = re.findall(r"\b[A-Z][a-z]+(?:\s[A-Z][a-z]+){0,2}\b", text)
    for m in regex_matches:
        candidates.add(m)
    ...
    def score(name):
        ...
        if not name.isupper():
            s += 5
        if any(name in ln for ln in lines[:5]):
            s += 8
        return s
...
def extract_education(text):
    ...
    for patt in DEGREE_PATTERNS:
        ...
        if m:
            tok = m.group().upper()
            mapping = {
                "BTECH": "B.Tech", "B.TECH": "B.Tech", "BE": "B.E", "B.E": "B.E",
                "BCA": "BCA", "BSC": "B.Sc", "B.SC": "B.Sc",
                "MTECH": "M.Tech", "M.TECH": "M.Tech", "MCA": "MCA",
                "MSC": "M.Sc", "M.SC": "M.Sc", "MBA": "MBA",
                "PHD": "PhD", "PH.D": "PhD"
            }
            degree = mapping.get(tok, tok.title())
            break
...
def extract_skills_from_pdf(uploaded_file):
    """
    Returns: {"name": ..., "education": ..., "skills": [...]}
    FAST + ROBUST + STREAMLIT CLOUD SAFE
    """
    text = extract_text_from_pdf(uploaded_file)
    cleaned = clean_text_before_name_extraction(text)
